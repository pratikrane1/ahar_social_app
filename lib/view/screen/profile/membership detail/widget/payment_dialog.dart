...
class PaymentDialog extends StatefulWidget {
  ...
  String? paidAmount;
  Profile? profileData;
  PaymentDialog({Key? key, required this.amount, required this.paidAmount,required this.profileData})
      : super(key: key);
...
class _PaymentDialogState extends State<PaymentDialog> {
  ...
  void handlePaymentSuccessResponse(PaymentSuccessResponse response) {
    ...
    print(res);
    Get.find<MembershipController>().transactionResponseRazorPay(widget.profileData!.id.toString(),amount.toString(),orderId.toString(),res, "Razarpay");
    showAlertDialog(
                      ...
                      child: AppButton(
                        onPressed: () async {
                          ...
                          if (_character == PaymentMode.mode1) {
                            ...
                            then((value) {
                            options = {
                              'key': '${widget.profileData!.paymentKey}',
                              'amount': amount,
                              'name': '${"Ahar Membership Renewal"}',
                              'order_id': value, // Generate order_id using Orders API
                              'description': 'Ahar Membership Renewal',
                              'retry': {'enabled': true, 'max_count': 1},
                              'send_sms_hash': true,
                              'prefill': {
                                'contact': '${widget.profileData!.mobileNo}',
